...
class TestRedisLiteConnection:
    """Tests for the RedisLiteConnection class."""

    class TestQueueFunctions:
        """Tests for accessing queues."""

        def test_list_queues_returns_a_set_of_queue_names(self, conn: RedisLiteConnection):
            """We can list queues."""
            assert conn.list_queues() == set()

            conn._connection.fcall("create_queue", 1, "test_queue")
            assert conn.list_queues() == {"test_queue"}

            conn._connection.fcall("create_queue", 1, "test_queue2")
            assert conn.list_queues() == {"test_queue", "test_queue2"}

        def test_create_queue_creates_a_queue(self, conn: RedisLiteConnection):
            """A queue is created."""
        ...
        def test_delete_queue_deletes_a_queue(self, conn: RedisLiteConnection):
            """A queue is deleted."""
            r = conn.create_queue("test_queue", 10, 0, 0)
            assert r
            assert conn._connection.sismember("QUEUES", "test_queue")
            conn.delete_queue("test_queue")
            assert not conn._connection.sismember("QUEUES", "test_queue")

        def test_delete_queue_is_idempotent(self, conn: RedisLiteConnection):
            """A queue is deleted."""
            assert not conn._connection.sismember("QUEUES", "test_queue")
            conn.delete_queue("test_queue")
            assert not conn._connection.sismember("QUEUES", "test_queue")
    ...
    class TestQueueMetadata:
        """Tests for getting/setting queue metadata."""

        def test_get_queue_info_returns_a_dict_of_metadata(self, conn: RedisLiteConnection):
        ...
        def test_set_queue_visibility_timeout_changes_the_value(self, conn: RedisLiteConnection):
        ...
        def test_set_queue_initial_delay_changes_value(self, conn: RedisLiteConnection):
        ...
        def test_set_queue_max_size_changes_value(self, conn: RedisLiteConnection):
    ...
    class TestPushMessage:
        ...
        def test_can_send_to_an_existing_queue(self, conn: RedisLiteConnection):
        ...
        def test_can_send_to_a_nonexisting_queue(self, conn: RedisLiteConnection):
        ...
        def test_can_override_the_initial_delay(self, conn: RedisLiteConnection):
        ...
        def test_can_send_metadata_with_message(self, conn: RedisLiteConnection):
            """You can send a message with metadata."""
            msg_id = conn.push_message("test_queue", "foo", metadata={"foo": "bar"})

            # metadata is stored as a msgpack blob
            msg_metadata = unpackb(conn._connection.hget("test_queue:Q", f"{msg_id}:metadata"))

            assert msg_metadata["foo"] == "bar"
            assert "sent" in msg_metadata

        def test_raises_error_if_metadata_is_not_dict(self, conn: RedisLiteConnection):
            """You can send a message with metadata."""
            with pytest.raises(TypeError):
                conn.push_message("test_queue", "foo", metadata="foo")

    class TestGetMessage:
        """Tests for getting messages."""

        def test_can_get_a_message_from_an_existing_queue(self, conn: RedisLiteConnection):
        ...
        def test_can_get_a_message_from_a_nonexisting_queue(self, conn: RedisLiteConnection):
            """You can get a message from a non-existing queue."""
            msg = conn.get_message("test_queue")
            assert msg is None
